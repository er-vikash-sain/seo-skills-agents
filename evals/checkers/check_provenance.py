...
Validates that every factual metric claim in a report explicitly cites a valid source file [Source: <path>],
verifies the source file exists, and asserts that the exact numeric token matches inside the cited source file content.
"""
            ...
            if not source_match:
                ...
                if not os.path.exists(source_file):
                    ...
                    if digits_match:
                        ...
                        # Exact word-boundary token check (\b<digits>\b) to prevent substring matching failures
                        token_pattern = re.compile(r'\b' + re.escape(raw_digits) + r'\b')
                        if not token_pattern.search(source_content):
                            errors.append(f"Line {line_num}: Claimed metric token '{raw_digits}' (from '{claimed_val}') not found as exact numeric token in source file ('{source_file}')")
